...
- `HIGODOT_SOLE_AUTHORING_AUTHORITY`: 채택 후 HiGodot만 Scene·Node·Resource·`project.godot` 설정 변경을 저작한다.
- `GUT_SOLE_TEST_AUTHORITY`: 채택 후 GUT 9.7.1만 GDScript 단위·통합 테스트 프레임워크 권위를 가진다.
- `ENTRY_GATE_FAIL_CLOSED`: 결정 원장·미확정/감사·이미지 검수·열린 PR exact HEAD 중 하나라도 누락·stale이면 작업 진입을 차단한다.

현재 HiGodot은 `PILOT_ONLY_NOT_PRODUCTION_AUTHORING_AUTHORITY`, GUT은 `NOT_INSTALLED_DESIGN_ONLY`다. 설계 승인과 별도 채택 PR 없이 둘을 READY로 승격하거나 제품 경로를 변경하지 않는다.

GUT 런타임은 Git 추적 파일을 수정할 수 없고, HiGodot은 `tests/gut/**`, `.gutconfig.json`, `addons/gut/**`를 수정할 수 없다. 같은 파일의 이중 권위와 출처 미상 변경은 실패 처리한다.

`READY`·`AWAITING` 같은 일반 상태 문자열만으로는 진입할 수 없다. 범위, current main SHA, Sheet 범위, 미실행 검증과 차단 이유가 기계 판독 가능한 상태로 함께 기록되어야 한다.
