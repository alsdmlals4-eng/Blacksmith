...
> 2026-09-13 scoped recovery approval: `BS-OPS-20260913-01` in the current production contract. For the exact replan-rule module and UID listed in `docs/operations/PROJECT_PROTECTED_CHANGE_APPROVAL.json`, the user explicitly permits read-only diagnosis and bounded recovery after the ordinary validator fails. Use the existing main CI-pinned `check_approved_project_operating_contract.py`, unchanged adopted Base v9.4.4/baseline, exact manifest and freshly verified current-task PR `approved-protected-change` metadata. Any other error or path mismatch remains fatal. Do not set external approval true without verifying that metadata. This is not a blanket router exception or authorization to weaken checks. Before each further implementation, state its plan, impact and evidence criteria first.
