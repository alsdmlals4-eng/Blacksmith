...
func test_initializer_run_identity_seed_and_timestamp_match_approved_contract() -> void:
	...
	var seed = envelope.active_run.get("run_rng_seed", -1)
	assert_true(seed is int, "run_rng_seed must be an integer")
	assert_true(int(seed) >= 0, "run_rng_seed must be unsigned")
	assert_true(int(seed) <= 4294967295, "run_rng_seed must fit u32")
