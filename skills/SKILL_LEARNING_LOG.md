...
## 2026-09-13 — 예상치와 실제 사건 증거 분리

- 원격 문서 검사에서 Active Context 상단 누적 요약이 기존 결정 링크를 첫2500자 밖으로 밀어냈다. 기존 검사 기준을 유지하고 현재 생산계약/결정 추적 링크를 제목 바로 아래 고정했다. 날짜별 보고보다 cold-start 진입점을 앞에 유지한다.

- 신규 태그점수를 기존 고객 결과에 즉시 끼워 넣으면 다른 content/결과축의 의미가 혼합된다. 현재 Blueprint 시험 모험을 순수 예상치 consumer로 연결하고 실제 저장 사건과 구분했다. raw지원16과 cap후15%p를 분리하여 상한으로 사라지는 기여를 설명한다.
- 미산출 비용의 기본0은 무료처럼 보인다. 실행 자격과 quote 유무를 구분하여 미선택에는 확인 필요를 표시한다. UI 회귀 RED→GREEN 및 전체279/2433으로 보호했다. 공용 Base 코드 변경 없이 프로젝트 소비처에서 해결.

## 2026-09-13 — 실제 선택·재실행과 저장 슬롯 경계

- 자동 시험 이후 실제 포인터 선택→강화→파일 저장→재실행을 별도로 확인했다. +9 시험 사전조건과 자연스러운 첫 플레이를 혼동하지 않는다. 화면 검수로 태그 요약 누락과 비검 장비에 검 손상 그림이 나오는 consumer 오류를 발견해 회귀 시험으로 보호했다.
- 구형 이어하기에 서비스만 바꾸면 후속 새 게임이 구형 슬롯을 덮어쓸 수 있다. 새 캠페인 서비스를 보존하고 새 시작 전에 복원한다. 대역의 구형 슬롯 쓰기0회 RED→GREEN 및 전체278/2414로 검증. Base 변경 대신 기존 프로젝트 구조를 보완했다.

## 2026-09-13 — 고객 선택과 실제 촉매 재고 연결

- 새 기획의 fire_heart와 실제 저장 자원 heart_of_flame은 같은 불의 심장이다. 이름만 보고 새 재고를 만들지 않고 precision adapter에서 명시 매핑했다. 5종 자격·실제 자원 소비·저장 실패 불변을 기존 action service로 검증했다.
- 고객 적합도의 가상 성공 비교와 실행 가능 여부는 분리한다. 재료가 없어도 효과 비교는 가능하지만 저장 서비스 실행을 허용하지 않는다. IV/슬롯 제한처럼 실제 불가능한 성장에는 가상 결과를 표시하지 않는다.
- 규칙 계산/저장 가능과 UI/플레이 연결은 별개다. 이번 서비스 연결은 전체 debug GUT273/2380으로 확인했지만 신규 공방 선택 UI·사건 적용·실제 캡처는 미완료다. 공용 Base 변경 없이 기존 exact approval gate와 거래 구조를 재사용했다.
