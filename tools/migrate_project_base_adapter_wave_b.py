...
def main() -> None:
    ...
    state = preserved_root["project_operating_state"]
    health["current_operating_decision"] = state["current_r2_decision"]
    health["current_stage"] = state["stage"]
    health["current_stage_status"] = state["stage_status"]
    health["product_implementation"] = state["product_implementation"]
    health["runtime_validation"] = state["new_r2_runtime_validation"]
    health["android_device_validation"] = state["android_device_validation"]
    health["human_playtest"] = state["human_playtest"]
    ...
    adapter = {
        ...
        "routing": {
            ...
            "project_routes": [
                normalized_route(route) for route in original["routing"]["project_routes"]
            ],
        },
        "schema_version": 1,
    ...
    mapping_rows = [
        ...
        ("/project_operating_state", "/adapter_migration/preserved_from_adapter/project_operating_state", "verbatim and current pointers advanced"),
...
The migration changes the Base connection contract and its generated views only. It does not edit `data/`, `scripts/`, `scenes/`, `assets/`, `addons/`, `project.godot`, Blacksmith gameplay canon, or Google Sheet cells. Historical PASS and NOT_RUN values are preserved verbatim and are not promoted.
""",
        encoding="utf-8",
    )
