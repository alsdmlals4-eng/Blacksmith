    ...
    commands = [
        (
            ...
            [
                python,
                "-c",
                "import pytest; assert pytest.__version__ == '8.3.5', pytest.__version__",
            ],
        ),
        (
            ...
            [
                ...
                "-m",
                "unittest",
                "tests/test_local_validation_fallback.py",
                "-v",
            ],
        ),
        (
        ...
        (
            "no-merge-conflicts-unittest",
            [python, "-m", "unittest", "tests/test_no_merge_conflicts.py"],
        ),
        (
            "no-merge-conflicts-scan",
        ...
        (
            ...
            [
                ...
                "-m",
                "unittest",
                "tests.test_archive_retention_governance",
                "-v",
            ],
        ),
    ]
            ...
            [
                ...
                (
                    ...
                    [
                        ...
                        "-m",
                        "unittest",
                        "tests/test_enhancement_balance_simulator.py",
                    ],
                ),
                (
                    "enhancement-simulator-contract",
                    [
                        python,
                        "tests/check_enhancement_balance_simulator_contract.py",
                    ],
                ),
            ]
        )
...
def python_matrix_commands(
    python: str,
    scope: str,
) -> list[tuple[str, list[str]]]:
    """Mirror python-validation.yml with include-operating-audit=false."""
    return _shared_python_commands(python, scope)
...
def python_commands(
    ...
    commands = _shared_python_commands(python, scope)
    commands[11:11] = [
        (
            "base-v9-adoption",
        ...
        (
            "thin-adapter",
            [python, "tests/test_project_base_adapter_thin_migration.py", "-v"],
        ),
        (
            "pr-diff-whitespace",
            ["git", "diff", "--check", f"{pr_base_sha}...HEAD"],
        ),
        (
            "operating-audit-runner",
            [
                python,
                "-m",
                "unittest",
        ...
        (
            ...
            [
                ...
                str(operating_base),
                "--report",
                "artifacts/base-adoption-report.json",
            ],
        ),
        (
            ...
            [
                python,
                str(contract_base / "tools" / "check_project_operating_contract.py"),
                "--project-root",
                ".",
                "--base-repository",
                str(contract_base),
...
def godot_commands(godot: str, repo: Path) -> list[tuple[str, list[str]]]:
    commands = [
        ...
        (
            "godot-import",
            [godot, "--headless", "--editor", "--path", str(repo), "--quit"],
        ),
    ]
    for scene in (
    ...
    commands.extend(
        (
            f"godot-{Path(path).stem}",
            [godot, "--headless", "--path", str(repo), "--script", path],
        )
        for path in suites
    )
    commands.extend(
        [
            (
                "godot-gut-import",
                [godot, "--headless", "--import", "--path", str(repo)],
            ),
            (
                "gut-cli",
                [
