...
```yaml
CURRENT_STAGE: R2_BATCH_006_MAIN_CANON_SCOPED_VERTICAL_SLICE_TASK2_ENTRY_GATE_PASS_RED_ALLOWED
...
ENTRY_STATE_GATE: PASS_SCOPED_TASK2_RED_ALLOWED
...
판정: `TASK2_DESIGN_APPROVED / INITIALIZER_AUTHORITY_RESOLVED / ENTRY_GATE_PASS_SCOPED_RED_ALLOWED`.
...
일반 제품 구현은 `BLOCKED`다. 버티컬 슬라이스는 R2 Batch 006이 승인한 namespace와 사용자 승인 Task에서만 허용한다. `BS-VS-INIT-20260808-01`은 Task 2의 initializer/save-replacement 범위만 추가하며 일반 제품 Gate를 열지 않는다.
...
Task 2 RED 허용은 `BS-VS-TASK2-20260807-01`과 `BS-VS-INIT-20260808-01`의 범위에만 적용한다. 이미지 목록의 열 정렬 복구나 engine-native UI 사용은 제품 이미지 생성·권리·가독성·런타임 Gate를 개방하지 않는다.
