...
- [x] 시뮬레이션 범위·입력·지표·판정 계약 등록
- [x] 실제 강화 규칙의 고정 roll 반례: 유지·보정·하락·파괴·폭주 이정표 차단·특수 비용·판매가
- [x] +10·+30·+50·+70·+100 × 균형·안정·폭주 기준선 15개 조합 × 1,000회 실행
- [x] 입력 커밋·JSON SHA-256·시드·난수 방식을 기계 판독 결과에 기록
- [x] 기준선 결과를 `KEEP / TUNE 후보 / TEST_IN_PLAY`으로 분리
- [ ] 성공률·비용·하락·파괴 중 한 변수군 후보안의 동일 시드 비교
- [ ] 자동 반복·자원 병목·보관함 중지·파괴 후 재시작 분포
- [ ] 실제 Godot 난수 시퀀스와 시뮬레이터 난수 시퀀스 대조
