...
POC v0.6.4 강화 실패 정책의 단일 정본과 데이터 의미 검증을 완료했다. 위험·가격 곡선 시뮬레이션 Skill Mode와 범위 계약을 준비했으며, 다음 제품 작업은 실제 런타임을 재현하는 기준선 시뮬레이터 구현·실행이다.

## 현재 상태

- 제품 단계: Prototype
- 작업 게이트: 강화 실패 정책 Integration·Completion PASS / 밸런스 시뮬레이션 준비 READY
...
- 밸런스 시뮬레이션 준비: `blacksmith-game-design: balance-simulation`, 자동 trigger, Work Mode 라우팅, Base 적용 프로필, Design Document Registry, `docs/BALANCE_SIMULATION_SCOPE.md` 동기화
- 밸런스 시뮬레이션 실행: 시뮬레이터 구현·기준선 분포·후보 수치 비교·결과 보고서 모두 NOT_RUN
...
- 밸런스 시뮬레이션 범위: `../../docs/BALANCE_SIMULATION_SCOPE.md`
...
1. `docs/BALANCE_SIMULATION_SCOPE.md`에 따라 실제 강화 판정 순서를 재현하는 시뮬레이터를 구현한다.
2. +10·+30·+50·+70·+100과 균형·안정·폭주·재료·자동 반복 기준선을 동일 시드로 실행한다.
3. 평균·중앙값·P90·P95·하락·파괴·복구 비용·재료 병목을 보고 `KEEP / TUNE / REJECT / TEST_IN_PLAY`로 판정한다.
4. `TUNE`이 승인된 경우 한 변수군만 조정하고 Godot·데이터·운영체계 회귀를 다시 실행한다.
5. 위험·가격 곡선 정합화 뒤 방문 검투사 판매를 구현한다.

## 미검증·위험

- 밸런스 시뮬레이터의 런타임 재현 정확도
- +10~+100 기준선 분포와 파괴 후 복구 비용
