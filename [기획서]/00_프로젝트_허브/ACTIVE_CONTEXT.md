...
> 모닥 최신: 힘을 잃고 공방의 조수로 머무는 인간형 불의 정령. 이번 모닥 시안은 사용자 허용에 따라 비픽셀 애니메이션 일러스트이며 `docs/design/candidates/modak-human-20260911/record.json` 참조. 아래10안은 이전 탐색 이력. 대장장이1번 선택 유지, 전체 게임 그림체 변경·제품 구현 승인으로 확대하지 않는다.
