...
> 역사 비교 입력: 이 보고서의 `R_BAND = 100`은 곡선 감도를 읽기 위한 정규화 입력이다. 현재 정규 테스트 데이터는 Decision31 JSON의 `USER_APPROVED_MUTABLE_TEST_BASELINE` 표이며, 사람 플레이 또는 새 밸런스 증거에 따라 수정할 수 있다.
