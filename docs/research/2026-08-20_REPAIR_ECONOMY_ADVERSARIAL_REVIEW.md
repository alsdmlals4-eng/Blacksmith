...
- 대상: `BS-ENHANCE-20260820-10`
- 상태: `PLAN_REVIEW_EVIDENCE / USER_APPROVED`
- Work Mode: `PLAN`
...
`REPAIR_REFERENCE_COST`는 시장 판매가가 아니라 안정된 구조 수리 참조값이며 최종 판매가·예술성·수식어·연대기·고객 수요·실제 다음 강화비에 직접 연동하지 않는다. 정확 계수와 band table은 출시 최종값이 아니다.
...
### 공격
순수 비례 비용이면 `CURRENT -1~-5` 같은 작은 손상이 거의 공짜다. 모든 실패 직후 수리하는 루틴이 생기면 `그대로 한 번 더` 판단이 사라진다.

### 대안
1. 비례 비용만 사용
2. 최소 수리 가능 손상 threshold
3. **고정 준비비 + 비례 비용 + 한 번의 공방 작업**

### 방어
3을 채택한다. 고정 `setup_fraction`과 `REPAIR_JOB` 기회비용을 두며 자동수리와 부분수리를 첫 Vertical Slice에서 제외한다.
...
### 방어
- 한 번의 `REPAIR_JOB`으로 CURRENT 전체를 MAX까지 회복한다.
- 중·대 손상의 일반 수리비는 동일 경험 밴드 일반 강화 1회 참조비용보다 대체로 낮게 시작한다.
- MAX를 고치지 않으므로 수리를 싸게 만들어도 장기 구조 리스크는 남는다.
- 정확 DAY_WORK_COST는 후속 Balance Decision에서 조정한다.
...
### 공격
`최종 판매가 × 손상률`을 쓰면 수식어·예술성·연대기·강화 적용 순서에 따라 수리비가 달라지는 비용 최적 순서가 생길 수 있다. `다음 강화비 × 손상률`도 특수 체크포인트 비용 폭증 때문에 같은 문제가 있다.
...
### 공격
수리량을 `(MAX-CURRENT)/MAX`로 계산하면 낮은 MAX 작품이 동일 절대 결손을 복구할 때 더 비싸진다. MAX가 낮은 작품은 이미 강화 성공 기대와 신규 강화 효과에 장기 페널티를 받고 있다.
...
### 공격
자동 마모·부분 수리 반복·수리 전용 재료·수리 RNG·MAX 일반 복원을 결합하면 강화보다 수리가 더 많은 시간을 먹는다.
...
`R` 자체는 기본 작품 구조/주재료/확보 위험 밴드별 안정된 data table로 두고, 실제 다음 특수 강화 비용을 런타임에서 직접 참조하지 않는다.
...
- 5회 전체 적대 검토 완료.
- `BS-ENHANCE-20260820-10 = C`: `USER_APPROVED`.
- 구조적 P0/P1 모순은 현재 발견되지 않음.
- 정확 `REPAIR_REFERENCE_COST` table, 골드/재료 분배, DAY_WORK_COST는 후속 Balance Decision에서 정교화.
