...
> Latest: user selected smith option1 (visual direction only). Ten new Modak appearance alternatives are in `docs/design/candidates/modak-ten-20260911/record.json`, USER_REVIEW_PENDING with intentional opaque review backgrounds. No runtime or PDF replacement. Earlier unresolved smith-selection notes below are superseded.

> Current visual review: user requested3young handsome anime-pixel smith alternatives and1fire-spirit Modak. `docs/design/BLACKSMITH_HUMAN_BLUEPRINT_PRODUCTION_20260911.md` and `docs/design/candidates/young-smith-spirit-20260911/record.json` own this bounded comparison. Four images are USER_REVIEW_PENDING, not clean runtime assets;2have baked checkerboards,2need alpha/halo refinement. No game or41-page PDF change.

> 2026-09-11 latest: integrated human Blueprint `docs/design/BLACKSMITH_HUMAN_BLUEPRINT_20260911.md` is the detailed review owner, derived PDF `exports/blacksmith_HUMAN_BLUEPRINT_20260911.pdf`. The user supplied a92-page structure-only example and explicitly resumed image production for actual game consumers. Preparation includes SWOT strategies, atlas, system/data/transaction contracts, exact early-test budget and implementation checklist. Old image-PAUSED statements below are historical. Product changes remain paused until final user planning approval. Seven asset families are candidates, not all-state ASSET_READY; motion grid/attachments/overlays and actual engine/device/human checks remain. Current-task PR371 stays Draft; other PR359/196 read-only. Base observation2f93e872d9ed4fa18018ac759b01acd7d34e9b58 does not replace adoptedv9.4.4.
...
> Current operational adaptation · 2026-09-03: `docs/operations/BLACKSMITH_BASE_CURRENT_ADAPTATION_WORK_CONTRACT_20260901.md` is the current owner for Base-current observation, project-adapted work modes, L1+ work receipts, legacy/context hygiene, and validation/PR readback routing. Base `main@850204b3e5de81a4045111b4a050c46c5a292b59` is a fresh-read reference only; Blacksmith's adopted Base release remains adapter-owned `v9.4.4 / 210ec78292fa12ed7563ba743b322dd36103ae4a` with no automatic pin change. The owner separates `PLAN`, `NONCODING_BUILD`, `GODOT_PRODUCT_BUILD`, and `REVIEW` without changing current product scope, GitHub-only canon, PR #196 protection, or Human/Android/accessibility/performance evidence ceilings.
...
and never mutates the stored result. `PR #286` adds the save-first customer-flow caller:
it resolves on a cloned envelope, saves that candidate, then presents that same stored result
only after save succeeds. The event scheduler and player-facing customer entry remain `NOT_RUN`.
