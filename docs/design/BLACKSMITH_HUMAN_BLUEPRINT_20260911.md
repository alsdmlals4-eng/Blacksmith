...
## 78. 모닥과 첫 납품 - 실제 합류·인계·재시작

2026-09-20 실제 Windows 게임 캡처. 격리 슬롯에서 포인터 합류→의뢰 납품→프로세스 재시작을 확인했다. 의뢰 수락/단조 완료값은 실제 service에 fixture로 입력했다. 단조 미니게임을 사람이 처음부터 완주한 증거는 아니다.

```runtimecaptures
../testing/modak-delivered-native-20260920.png|인계 직후: 모닥의 납품 반응, 함께 납품한 날1일. 기존 보수만 지급하고 추가 성장 보상은 없다.
../testing/modak-restarted-native-20260920.png|재시작: EARLY·합류 영업일1·납품일1 유지. 이전 축하 재생 없이 중립 안내로 복원한다.
```

합류는 선택이며 과거 저장을 읽는 것만으로 합류하지 않는다. 납품일은 기존 인계와 같은 저장에 기록한다. 수락·빈날·재열람·합류 전 납품은 세지 않는다. 성장 연도와 LATER 발동은 아직 잠겨 있다.

검증: 전체 debug GUT385/4959 assertions PASS, 경고52개 별도. 독립 검토2회의 불확실 합류 해제·파괴 반응·강화/납품 실패 뒤 축하 잔존4건을 재현/교정했다. 현재는 텍스트 조수 반응이며 배우 모션·Android·사람 재미 검수는 미완료다.

---

## 79. 모닥 상태 아틀라스 후보와 남은 구현 체크리스트

승인된 어린 정령 외형을 유지한 네 자세 후보. 왼쪽부터 대기 / 도움 / 기쁨 / 걱정이다. 아래는 실제 이미지 도구의 크로마키 원본이며 인게임 캡처나 최종 승인 자산이 아니다.

![모닥 네 자세 크로마키 후보](candidates/modak-first-forge-20260920/modak-states-chroma.png)

| 상태 | 전달할 정보 | 적용 조건·현재 상태 |
|---|---|---|
| 대기 | 함께 일하는 어린 조수 | 합류 확인 후. 현재는 텍스트 표시 |
| 도움 | 도구를 건네는 행동 | 손·도구 접점/복귀 프레임 제작 전 |
| 기쁨 | 저장으로 확정된 성공·납품 | 중복 억제·스킵 구현, 그림 연결 전 |
| 걱정 | 손상/파괴 뒤 수습 | 실패 후 중립 복귀 구현, 그림 연결 전 |

QA FAIL: RGB1774×887은 균등4칸 규격 불일치, alpha 추출은 잔여색/반투명 내부. Aseprite 후보 포장만 완료, runtime 승격0. 남음: alpha·프레임·Portrait·손/도구 모션·최종 미술·성장 달력·HUMAN/Android. 구현 체크리스트와 PR388 검토/CI/main 상태는 생산계약에서 확인한다.
