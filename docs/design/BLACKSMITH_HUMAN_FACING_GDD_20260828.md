...
정밀 강화는 매 10단계의 경계에서 열린다. `+9 → +10`부터 `+99 → +100`까지 정확히 열 번이다. 각 시도에는 선택한 정밀 촉매 1개가 필요하며, 성공할 때만 `+1`과 함께 태그 행동 하나가 남는다.
...
정밀 강화에서 성공 한 번에는 태그 행동도 정확히 하나만 적용된다. 성공·실패 유지·실패 손상으로 **저장된 시도**는 선택한 촉매 1개를 소비한다. 실패·유지, 실패·손상, 또는 막힌 시도는 태그, 단계, 효과를 전혀 바꾸지 않는다. 촉매가 없거나 저장이 실패하면 골드·보강재·촉매와 작품은 모두 종전 상태다.
...
| 정밀 촉매 | 방식 | 무기 태그 | 무기에 남는 변화 |
|---|---|---|---|
| 불의 심장 | 날 세우기 | 불의 심장 · 예리함 | 역할 능력치 +3 |
| 불의 심장 | 경량 담금 | 불의 심장 · 경량 | 무게 -3, 0 아래로는 내려가지 않음 |
| 대지의 결정 | 날 세우기 | 대지의 결정 · 예리함 | 역할 능력치 +3 |
| 대지의 결정 | 경량 담금 | 대지의 결정 · 경량 | 무게 -3, 0 아래로는 내려가지 않음 |

정밀 촉매 → 정밀강화 방식 → 태그 키워드의 순서로 작품의 방향을 읽는다. `불의 심장`과 `대지의 결정`은 각각 1개씩 실제 공방 자원으로 소비되며, 선택 화면은 보유량과 이번 시도의 소모량을 먼저 보여 준다. 이 선택은 내구도를 바꾸지 않으며, 등급이나 사건 태그를 바꾸지도 않는다.
...
정밀 강화는 별도 배경 그림을 쓰지 않는다. 태그 선택, 촉매별 보유량·1개 소모, 성공·실패 확률, 비용, 결과는 네이티브 Godot UI로만 보여 주며, 검·방패·활만 각 +10 단위에서 태그를 추가하거나 강화한다. 촉매 재고가 부족하면 실행 버튼은 비용·굴림 전에 비활성화된다. 갑옷과 투구는 `+9 → +10`에서 비용·굴림·변경 전에 차단된다.
...
- 소모형 정밀 촉매·V5 resource migration·저장 원자성: `docs/decisions/BS-ENHANCE-20260901-40_CONSUMABLE_PRECISION_CATALYST_RESOURCES.md`
