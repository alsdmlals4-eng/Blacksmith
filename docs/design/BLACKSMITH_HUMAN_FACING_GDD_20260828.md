...
- 문서 상태: `CURRENT_HUMAN_FACING_GDD / KOREAN_PRIMARY / 2026-08-30 KST`
- 독자: 공방의 한 판이 어떤 선택과 결과를 만드는지 알고 싶은 플레이어와 협업자.
- 읽는 경계: `사람용 본문 = 플레이 경험의 이해와 의사결정`, `개발 참고 = 구현·데이터·테스트 추적`이다. 이 문서는 내부 구현 이름이나 검사 절차를 설명하지 않는다.
...
- 성공은 언제나 정확히 `+1`이다.
- 결과는 `SUCCESS`, `FAILED_HOLD`, `FAILED_DAMAGE` 셋 중 하나다. 단계 하락이나 별도 치명 결과는 없다.
- `FAILED_HOLD`는 강화 단계와 태그를 바꾸지 않는다. `FAILED_DAMAGE`는 단계와 태그를 바꾸지 않고 현재 내구도만 줄일 수 있다.
...
아래 삽화는 게임 규칙이나 완성된 화면의 증거가 아니다. 각 장면은 텍스트와 버튼을 이미지 안에 굽지 않고, 게임의 읽기 쉬운 조작층 뒤에서 공방의 맥락을 만드는 배경 또는 사건 삽화다.

| 장면 | 쓰임 | 현재 확인된 상태 |
|---|---|---|
| 메인 메뉴 | 공방에 들어가 새 작업 또는 이어 하기를 고르는 배경 | 사용자 잠금·정본 등록·구현 기계 검증 |
| 정밀 강화 공방 | 열 번의 정밀 선택이 일어나는 중립적 작업대 배경 | 사용자 잠금·정본 등록·구현 기계 검증 |
| 고객 결과 | 실제 사용 뒤 돌아온 작품의 결과 맥락 | 사용자 잠금·정본 등록·구현 기계 검증 |

세 삽화는 런타임 스크린샷이 아니다. 실제 클라이언트, Android, 접근성, 사람의 시각 검수는 모두 `NOT_RUN`이다.
...
| 세 삽화의 등록·연결 | 사용자 잠금·정본 등록·구현 기계 검증 | 실제 화면에서 잘 읽힌다는 주장 |
| 자동 검증 | 기계적 계약과 연결 상태의 증거 | Godot 클라이언트, Android, 접근성, 성능 PASS |
| 사람 경험 | `NOT_RUN` | 사람이 STOP OR PUSH와 작품의 생애를 즐긴다는 주장 |
| 출시 | `NOT_RUN` | 출시 준비 또는 권리·플랫폼 PASS |

내구도·수리·고객 사건의 일부 수치는 현재 테스트 예산이다. 사람 플레이 검수는 이번 계약의 완료 조건이 아니다. 실제로 수행될 때까지 `NOT_RUN`이다. 5강 단위의 성장감은 관찰할 수 있지만, 열 번의 정밀 관문과 별개의 태그 선택을 대신하지 않는다.
