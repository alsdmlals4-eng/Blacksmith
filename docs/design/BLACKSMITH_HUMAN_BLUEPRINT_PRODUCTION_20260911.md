...
- [ ] EXCHANGE / AC1: GUT `tests/gut/unit/vertical_slice/test_vs_replan_tag_save.gd` first checks service presence then real save purchase: Gold2000→1000/fire+1; repeat same sequence remains1000; new earth purchase→0/earth+1; next BLOCKED. Validate malformed receipts/stale and failure fixtures; observe RED before authoring.
- [ ] EXCHANGE / AC2: after exact4-path manifest/current-task PR label readback, HiGodot creates service+UID and patches SaveEnvelope/workshop. UI native confirm/cancel≥96logical touch height; quote owns price; campaign_saved updates actual resources. Observe GREEN then full GUT/Python regression.
...
Current state: PLAN_READY / implementation and native evidence NOT_RUN. Next action: RED and exact protected delivery registration. Learning: reused save/confirmation pattern; no new Base promotion or unproven framework.
