...
최신 사용자 요청은 남은 작업과 명세 준비다. 이 문서는 기존 승인 방향을 실행 가능한 작업으로 분해하는 책임 원본이며, 새 경제·달력의 최종 승인이나 제품 구현 완료 선언이 아니다. 제작 계약이 실행 경계를, 기존 블루프린트가 게임 방향을 계속 소유한다. 사람용 블루프린트65~72절은 이 명세의 요약·체크리스트다. 별도 HTML PM은 만들지 않는다.
...
모든 단계 공통: fresh authority/consumer → 해당 범위 조사 → exact 보호 경로 등록 → RED → 최소 구현 → GREEN → 두 차례 적대 검토 → 실제 실행 → 블루프린트 체크리스트 → 정상 PR/정확 HEAD CI/main readback. 이번 요청에서는 명세까지 준비하며 위 구현을 시작했다고 기록하지 않는다.
