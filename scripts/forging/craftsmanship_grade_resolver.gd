...
func _init(config: Dictionary = {}) -> void:
	...
	for item: Variant in source.get("grades", []):
...
func resolve(precision_result_id: String, roll: float) -> Dictionary:
	var distribution: Dictionary = Dictionary(distributions.get(precision_result_id, distributions.get("STANDARD", {})))
	if distribution.is_empty():
		return {}
	var normalized_roll: float = clampf(roll, 0.0, 0.999999)
	var cursor: float = 0.0
	var ordered_ids: Array[String] = _ordered_grade_ids()
	for grade_id: String in ordered_ids:
		cursor += float(distribution.get(grade_id, 0.0))
		if normalized_roll < cursor:
			return _grade_result(grade_id, precision_result_id)
	var fallback_id: String = ordered_ids.back() if not ordered_ids.is_empty() else ""
...
func normalize_record(record: Dictionary) -> Dictionary:
	...
	var legacy_quality_id: String = str(normalized.get("quality_id", "STANDARD"))
	var precision_id: String = str(normalized.get("precision_result_id", legacy_quality_id))
	if precision_id == "":
		precision_id = "STANDARD"
	var grade_id: String = str(normalized.get("craftsmanship_grade_id", ""))
	var ordered_ids: Array[String] = _ordered_grade_ids()
	if grade_id == "" or not grade_by_id.has(grade_id):
		grade_id = "STANDARD" if grade_by_id.has("STANDARD") else (ordered_ids[0] if not ordered_ids.is_empty() else "")
	var grade: Dictionary = Dictionary(grade_by_id.get(grade_id, {}))
...
func _ordered_grade_ids() -> Array[String]:
	...
	for grade_id: Variant in grade_by_id:
