...
static func create(
	...
	entry_occurred_at_game_day: int,
	...
	entry.occurred_at_game_day = entry_occurred_at_game_day
...
static func from_dict(value: Dictionary) -> VSLedgerEntry:
	...
	entry.occurred_at_game_day = int(value.get("occurred_at_game_day", -1))
...
func _validate() -> void:
	...
	if occurred_at_game_day < 0:
		validation_errors.append("INVALID_LEDGER_OCCURRED_DAY")
