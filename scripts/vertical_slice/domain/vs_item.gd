...
static func from_dict(value: Dictionary) -> VSItem:
	...
	if item.schema_version != SCHEMA_VERSION:
		item.validation_errors.append("UNSUPPORTED_ITEM_SCHEMA:%d" % item.schema_version)

	item.uid = str(value.get("uid", ""))
	item.birth_rng_seed = int(value.get("birth_rng_seed", 0))
	...
	item.artistry = int(value.get("artistry", 0))
	item.raw_role_stat = int(value.get("raw_role_stat", 0))
	item.weight_point = int(value.get("weight_point", 0))
	item.function_capacity = int(value.get("function_capacity", 0))
	...
	item.enhancement_level = int(value.get("enhancement_level", 0))
	item.enhancement_failure_streak = int(value.get("enhancement_failure_streak", 0))
...
func _validate_values() -> void:
	...
	if primary_material_id.is_empty():
		validation_errors.append("MISSING_PRIMARY_MATERIAL")
	if equipment_group.is_empty():
		validation_errors.append("MISSING_EQUIPMENT_GROUP")
	if role_profile.is_empty():
		validation_errors.append("MISSING_ROLE_PROFILE")
	if crafting_grade.is_empty():
		validation_errors.append("MISSING_CRAFTING_GRADE")
	if artistry < 0:
