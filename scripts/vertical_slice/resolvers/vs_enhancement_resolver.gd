...
func resolve_with_rolls(item, target_level: int, rolls: Dictionary) -> Dictionary:
	...
	if bool(attempt["guaranteed"]) or float(rolls.get("success_roll_percent", 0.0)) < float(attempt["final_success_percent"]):
		...
		return {"outcome": "SUCCESS", "target_level": target_level, "band": str(attempt["band"])}
...
func _apply_success(item, target_level: int) -> void:
	...
	if target_level == 10 and str(item.catalyst_affix).is_empty(): item.catalyst_affix = "PRECISION_KEYWORD_PENDING_CONTENT"
