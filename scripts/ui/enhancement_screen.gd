...
func build_weapon_record() -> Dictionary:
	...
	return {
		...
		"fever_value_bonus": float(snapshot.get("fever_value_bonus", 0.0)),
		"fever_value_multiplier": float(snapshot.get("fever_value_multiplier", 1.0)),
		"crafting_value_multiplier": float(snapshot.get("crafting_value_multiplier", snapshot.get("quality_value_multiplier", 1.0))),
		"forging_completed_during_fever": bool(snapshot.get("forging_completed_during_fever", false)),
